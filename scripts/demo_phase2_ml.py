...
async def run_phase2_demo():
    ...
    async with AsyncClient(transport=transport, base_url="http://localhost:8000") as client:
        ...
        cls_res = await client.post("/api/v1/ml/train/classification", json={
        ...
        console.print(f"   [green][OK][/green] Model: [bold]{cls_data['model_id']}[/bold] | Accuracy: [cyan]{eval_metrics['accuracy']:.4f}[/cyan] | F1 Macro: [cyan]{eval_metrics['f1_macro']:.4f}[/cyan] | ROC-AUC: [cyan]{eval_metrics['roc_auc']:.4f}[/cyan]")

        # Test live inference
        cls_infer = await client.post("/api/v1/ml/predict", json={
            "model_id": f"churn_rf_{run_id}",
            "records": [
        ...
        console.print(f"   [green][OK][/green] Live Inference: Predictions={cls_pred_data['predictions']} | Probabilities={cls_pred_data['probabilities']} | Latency: [bold magenta]{cls_pred_data['latency_ms']} ms[/bold magenta]")
        results_summary.append(("Classification", cls_data["model_id"], f"F1: {eval_metrics['f1_macro']:.3f}, AUC: {eval_metrics['roc_auc']:.3f}", f"{cls_pred_data['latency_ms']} ms"))
        ...
        for i in range(120):
            ...
            price = sqft * 180.0 + beds * 15000.0 + (50000.0 if loc == "Urban" else 0.0) + float(np.random.normal(0, 8000))
        ...
        reg_res = await client.post("/api/v1/ml/train/regression", json={
            "project_id": project_id,
        ...
        console.print(f"   [green][OK][/green] Model: [bold]{reg_data['model_id']}[/bold] | R2 Score: [cyan]{reg_metrics['r2_score']:.4f}[/cyan] | RMSE: [cyan]${reg_metrics['rmse']:,.2f}[/cyan] | MAE: [cyan]${reg_metrics['mae']:,.2f}[/cyan]")

        reg_infer = await client.post("/api/v1/ml/predict", json={
            "model_id": f"price_gb_{run_id}",
            "records": [{"sqft": 1850.0, "bedrooms": 3, "location": "Suburban"}],
        }, headers=headers)
        reg_pred_data = reg_infer.json()
        console.print(f"   [green][OK][/green] Live Inference: Predicted Price=${reg_pred_data['predictions'][0]:,.2f} | Latency: [bold magenta]{reg_pred_data['latency_ms']} ms[/bold magenta]")
        results_summary.append(("Regression", reg_data["model_id"], f"R2: {reg_metrics['r2_score']:.3f}, RMSE: ${reg_metrics['rmse']:,.0f}", f"{reg_pred_data['latency_ms']} ms"))
        ...
        anom_res = await client.post("/api/v1/ml/train/anomaly", json={
            "project_id": project_id,
        ...
        console.print(f"   [green][OK][/green] Model: [bold]{anom_data['model_id']}[/bold] | Detected Anomalies: [cyan]{anom_metrics['detected_anomalies']} / {anom_metrics['total_samples']}[/cyan] ({anom_metrics['anomaly_percentage']}%)")

        anom_infer = await client.post("/api/v1/ml/predict", json={
            "model_id": f"fraud_iforest_{run_id}",
            "records": [
        ...
        console.print(f"   [green][OK][/green] Live Inference: Status={status_labels} | Scores={anom_pred_data['anomaly_scores']} | Latency: [bold magenta]{anom_pred_data['latency_ms']} ms[/bold magenta]")
        results_summary.append(("Anomaly Detection", anom_data["model_id"], f"Detected: {anom_metrics['anomaly_percentage']}%", f"{anom_pred_data['latency_ms']} ms"))
        ...
        fc_res = await client.post("/api/v1/ml/train/forecasting", json={
            "project_id": project_id,
        ...
        console.print(f"   [green][OK][/green] Model: [bold]{fc_data['model_id']}[/bold] | WAPE: [cyan]{fc_metrics['wape']:.4f}[/cyan] | RMSE: [cyan]{fc_metrics['rmse']:.2f}[/cyan] | R2: [cyan]{fc_metrics['r2_score']:.4f}[/cyan]")

        fc_infer = await client.post("/api/v1/ml/predict", json={
            "model_id": f"demand_forecast_{run_id}",
            "records": [{"dummy": 0}],
            "horizon": 7,
        }, headers=headers)
        fc_pred_data = fc_infer.json()
        console.print(f"   [green][OK][/green] Live 7-Day Trajectory Forecast: {[round(x, 1) for x in fc_pred_data['predictions']]} | Latency: [bold magenta]{fc_pred_data['latency_ms']} ms[/bold magenta]")
        results_summary.append(("Forecasting", fc_data["model_id"], f"WAPE: {fc_metrics['wape']:.3f}, R2: {fc_metrics['r2_score']:.3f}", f"{fc_pred_data['latency_ms']} ms"))
